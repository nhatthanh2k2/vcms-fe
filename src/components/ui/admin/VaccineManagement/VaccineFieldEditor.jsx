...
import 'quill/dist/quill.snow.css' // Import kiểu giao diện

export const VaccineFieldEditor = ({ label, items, setItems }) => {
    const editorRef = useRef(null) // Tham chiếu tới container của Quill
    const quillRef = useRef(null) // Lưu trữ instance của Quill

    // Chuyển đổi `items` từ dạng mảng sang HTML
    const formatItemsToHTML = (items) => {
        return items.map((item) => `<p>${item}</p>`).join('')
    }

    // Chuyển đổi HTML từ Quill về dạng mảng
