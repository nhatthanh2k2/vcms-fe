...
export const DeleteVaccineModal = ({
    ...
    const handleDeleteVaccine = async () => {
        try {
            const response = await vaccineService.deleteVaccine(vaccineSelected?.vaccineId)
            ...
            if (error.response)
                if (error.response.status === 400) MyToast('warn', 'Không thể xóa  vắc xin.')
        }
    }
    return (
        <Modal
        ...
        >
            <div className="space-y-3">
                <span className="font-semibold"> Tên vắc xin: </span>
                <span>{vaccineSelected?.vaccineName}</span>
            </div>
        </Modal>
    )
}
